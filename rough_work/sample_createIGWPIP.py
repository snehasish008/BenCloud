...
#UnderlayVnet.json file needs to be created manually as per teh requirement to deploy the Underlay
...
jdata = open('AzDeploy_IGW_PIP.json').read()
params = json.loads(jdata)
string=''
os.system('export rg=%s' %params['resource_group'])
...
if file_list:
    ...
    os.system('az group create --name $rg --location "%s"' %params['location'])

os.system('az group deployment create --resource-group $rg --template-file InternetAndPublicIpGateways.json --parameters InternetAndPublicIpGateways.parameters.%s.json' %params['customer'])
